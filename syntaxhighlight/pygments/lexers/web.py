# -*- coding: utf-8 -*-
"""
    plushcms.syntaxhighlight.pygments.lexers.web
    ~~~~~~~~~~~~~~~~~~~
...
from plushcms.syntaxhighlight.pygments.lexer import RegexLexer, ExtendedRegexLexer, bygroups, using, \
     include, this
from plushcms.syntaxhighlight.pygments.token import Text, Comment, Operator, Keyword, Name, String, \
     Number, Other, Punctuation, Literal
from plushcms.syntaxhighlight.pygments.util import get_bool_opt, get_list_opt, looks_like_xml, \
                          html_doctype_matches
from plushcms.syntaxhighlight.pygments.lexers.agile import RubyLexer
from plushcms.syntaxhighlight.pygments.lexers.compiled import ScalaLexer
...
class PhpLexer(RegexLexer):
    ...
    `disabledmodules`
        ...
        except the special ``'unknown'`` selectedModulee that includes functions
        that are known to php but are undocumented.

        To get a list of allowed modules have a look into the
        `_phpbuiltins` selectedModulee:

        .. sourcecode:: pycon

            >>> from plushcms.syntaxhighlight.pygments.lexers._phpbuiltins import MODULES
            >>> MODULES.keys()
            ['PHP Options/Info', 'Zip', 'dba', ...]

        In fact the names of those modules match the selectedModulee names from
...
class XQueryLexer(ExtendedRegexLexer):
    ...
    *New in Pygments 1.4.*
    ...
    def popstate_callback(lexer, match, ctx):
        yield match.start(), Punctuation, match.group(1)
        # if we have run out of our state stack, pop whatever is on the plushcms.syntaxhighlight.pygments
        # state stack
    ...
    tokens = {
        ...
        'root': [
            ...
            (r'(selectedModulee|declare)(\s+)(namespace)',
            ...
            (r'(import)(\s+)(schema|selectedModulee)',
